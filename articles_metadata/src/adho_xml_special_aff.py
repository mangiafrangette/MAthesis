...
def parse(file_path):
    ...
    authors_path = './/{http://www.tei-c.org/ns/1.0}titleStmt/{http://www.tei-c.org/ns/1.0}author'
    author_name = './/{http://www.tei-c.org/ns/1.0}name/{http://www.tei-c.org/ns/1.0}forename'
    author_family = './/{http://www.tei-c.org/ns/1.0}name/{http://www.tei-c.org/ns/1.0}surname'
    author_affiliation = './/{http://www.tei-c.org/ns/1.0}name/{http://www.tei-c.org/ns/1.0}affiliation/{http://www.tei-c.org/ns/1.0}name'
    ...
    # format dates
    if python_dict["date"] is not None:
        if "T" in python_dict["date"]:
...
def write_new_json():
    ...
    path = '../data/xml_files/adho_conferences/adho_papers/special_affiliations'
    ...
    with open(f"../data/json_files/no_country_dataset/ms_ADHO_Conference_special_affs.json", "w", encoding="utf-8") as fd:
        fd.write("[")
        # comment 125-127 and uncomment the following for one file test
        for file_xml in filenames: 
            json.dump(parse(f'{path}/{file_xml}'), fd, ensure_ascii=False)
            fd.write(",")
        #uncomment the following to test with one file
        """ json.dump(parse(f'{path}/WILMS_Lotte_Bridging_the_Gap_between_the_National_Libra.xml'), fd, ensure_ascii=False)
        fd.write(",") """
        fd.write("]")
...
list_aff("../data/json_files/no_country_dataset/ms_ADHO_Conference_special_affs.json")
