...
def parse_and_write(file_path):
    ...
    if python_dict["date"] is not None:
        if "T" in python_dict["date"]:
            ...
            python_dict["date"] = None
        """ elif parsed_xml.find(".//{http://www.tei-c.org/ns/1.0}sourceDesc") is not None:
            sourcedesc = parsed_xml.find(".//{http://www.tei-c.org/ns/1.0}sourceDesc")
            print(python_dict['article_title'])
            print(sourcedesc.tostring())
            #print(sourcedesc.find(".//{http://www.tei-c.org/ns/1.0}date").attrib['when'])
            
            
            if parsed_xml.find(".//{http://www.tei-c.org/ns/1.0}sourceDesc//{http://www.tei-c.org/ns/1.0}date").attrib['when'] is not None:
                other_date = parsed_xml.find(".//{http://www.tei-c.org/ns/1.0}date").attrib['when']
                print(other_date)
                python_dict["date"] = other_date """ 
